...
func TestHandleGetWidgets(t *testing.T) {
	cleanup := setupTest(t)
	defer cleanup()

	db.AddWidget("W1", "http://w1.url", "markus")
	db.AddWidget("W2", "http://w2.url", "andrea")

	t.Run("all widgets", func(t *testing.T) {
		req := httptest.NewRequest("GET", "/api/widgets", nil)
		rr := httptest.NewRecorder()
		api.HandleGetWidgets(rr, req)

		if rr.Code != http.StatusOK {
			t.Errorf("expected 200, got %d", rr.Code)
		}
		var widgets []db.Widget
		json.NewDecoder(rr.Body).Decode(&widgets)
		if len(widgets) != 2 {
			t.Errorf("expected 2 widgets, got %d", len(widgets))
		}
	})

	t.Run("filtered by profile", func(t *testing.T) {
		req := httptest.NewRequest("GET", "/api/widgets?profile=markus", nil)
		rr := httptest.NewRecorder()
		api.HandleGetWidgets(rr, req)
		...
		var widgets []db.Widget
		json.NewDecoder(rr.Body).Decode(&widgets)
		if len(widgets) != 1 || widgets[0].Name != "W1" {
			t.Errorf("expected 1 markus widget, got %v", widgets)
		}
	})
}

func TestHandleCreateWidget(t *testing.T) {
	cleanup := setupTest(t)
	defer cleanup()

	t.Run("success", func(t *testing.T) {
		body := `{"name":"NewW","url":"http://neww.url","profile":"markus"}`
		req := httptest.NewRequest("POST", "/api/widgets", strings.NewReader(body))
		req.Header.Set("Content-Type", "application/json")
		rr := httptest.NewRecorder()
		api.HandleCreateWidget(rr, req)

		if rr.Code != http.StatusCreated {
			t.Errorf("expected 201, got %d", rr.Code)
		}
		widgets, _ := db.GetWidgets("markus")
		if len(widgets) != 1 || widgets[0].Name != "NewW" {
			t.Errorf("widget not created: %v", widgets)
		}
	})

	t.Run("bad json", func(t *testing.T) {
		req := httptest.NewRequest("POST", "/api/widgets", strings.NewReader("not-json"))
		rr := httptest.NewRecorder()
		api.HandleCreateWidget(rr, req)
...
func TestHandleGetWidget(t *testing.T) {
	cleanup := setupTest(t)
	defer cleanup()

	db.AddWidget("GetMe", "http://get.me", "markus")
	widgets, _ := db.GetAllWidgets()
	wID := widgets[0].ID

	r := chi.NewRouter()
	r.Get("/api/widgets/{id}", api.HandleGetWidget)

	t.Run("found", func(t *testing.T) {
		req := httptest.NewRequest("GET", fmt.Sprintf("/api/widgets/%d", wID), nil)
		rr := httptest.NewRecorder()
		r.ServeHTTP(rr, req)
		...
		if w.Name != "GetMe" {
			t.Errorf("expected GetMe, got %s", w.Name)
		}
	})

	t.Run("not found", func(t *testing.T) {
		req := httptest.NewRequest("GET", "/api/widgets/99999", nil)
		rr := httptest.NewRecorder()
		r.ServeHTTP(rr, req)
...
func TestHandleUpdateWidget(t *testing.T) {
	cleanup := setupTest(t)
	defer cleanup()

	db.AddWidget("PatchMe", "http://patch.me", "markus")
	widgets, _ := db.GetAllWidgets()
	wID := widgets[0].ID

	r := chi.NewRouter()
	r.Patch("/api/widgets/{id}", api.HandleUpdateWidget)

	newName := "Patched"
	body, _ := json.Marshal(map[string]string{"name": newName})
	req := httptest.NewRequest("PATCH", fmt.Sprintf("/api/widgets/%d", wID), bytes.NewReader(body))
	req.Header.Set("Content-Type", "application/json")
	rr := httptest.NewRecorder()
	r.ServeHTTP(rr, req)
...
func TestHandleDeleteWidgetAPI(t *testing.T) {
	cleanup := setupTest(t)
	defer cleanup()

	db.AddWidget("DelMe", "http://del.me", "markus")
	widgets, _ := db.GetAllWidgets()
	wID := widgets[0].ID

	r := chi.NewRouter()
	r.Delete("/api/widgets/{id}", api.HandleDeleteWidgetAPI)

	req := httptest.NewRequest("DELETE", fmt.Sprintf("/api/widgets/%d", wID), nil)
	rr := httptest.NewRecorder()
	r.ServeHTTP(rr, req)
	...
	after, _ := db.GetAllWidgets()
	if len(after) != 0 {
		t.Errorf("expected 0 widgets after delete, got %d", len(after))
	}
}
...
func TestHandleReorderWidgets(t *testing.T) {
	...
	db.AddWidget("W1", "http://w1.url", "markus")
	db.AddWidget("W2", "http://w2.url", "markus")
	widgets, _ := db.GetAllWidgets()

	items := []map[string]int{
		{"id": widgets[0].ID, "sort_order": 1},
		{"id": widgets[1].ID, "sort_order": 0},
	}
	body, _ := json.Marshal(items)

	req := httptest.NewRequest("PATCH", "/api/widgets/reorder", bytes.NewReader(body))
	req.Header.Set("Content-Type", "application/json")
	rr := httptest.NewRecorder()
	api.HandleReorderWidgets(rr, req)
